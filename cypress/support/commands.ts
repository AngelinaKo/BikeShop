...
Cypress.Commands.add("addProductToCart", (product) => {
  cy.get(`#add-to-cart-${product}`).contains("Add to cart").click();
});

Cypress.Commands.add("checkIfCartIconShowsNumberOfItems", (number) => {
  cartIcon().contains(number);
});
...
Cypress.Commands.add("checkProductInOwerview", (product) => {
  cy.get(".cart_item").contains(product).scrollIntoView().should("be.visible");
});
...
Cypress.Commands.add("checkIfItemPurchasedSuccessfully", () => {
  cy.contains("THANK YOU FOR YOUR ORDER").scrollIntoView().should("be.visible");
});
